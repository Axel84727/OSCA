...
T_File *t_file_create_vector() {
    ...
    if (tf == NULL) {
        perror("Failed to allocate T_File");
        exit(EXIT_FAILURE);
    }
    tf->size = 0;
    tf->capacity = 16;
    tf->array = (Content *)malloc(sizeof(Content) * tf->capacity);
    if (tf->array == NULL) {
...
char *t_file_input(FILE *fp) {
    ...
    while (EOF != (ch = fgetc(fp))) {
        ...
        tf->array = (Content *)realloc(tf->array, (tf->size + 1) * sizeof(Content));
        if (tf->array == NULL) {
            perror("Failed to reallocate array");
            exit(EXIT_FAILURE);
        }
        tf->array[tf->size].data = (char)ch; // Ajusta esto según el tipo de datos
        tf->size++;
    }
    return (char *)tf->array; // Ajusta esto según el tipo de retorno que necesites
}typedef char Content;
...
T_File *t_file_create_vector() {
    ...
    if (tf->array == NULL) {
        perror("Failed to allocate array");
        free(tf);  // Liberar la memoria asignada a tf
        exit(EXIT_FAILURE);
    }
    return tf;
}// Estructura para almacenar el contenido del archivo
typedef char Content;
...
// Función para crear un vector de T_File
T_File *t_file_create_vector() {
    T_File *tf = (T_File *)malloc(sizeof(T_File));
    if (tf == NULL) {
        perror("Failed to allocate T_File");
...
// Función para destruir el vector de T_File y liberar memoria
void t_file_destroy_vector(T_File *tf) {
    free(tf->array);
    free(tf);
}

// Función para leer la entrada de un archivo
char *t_file_input(FILE *fp) {
    static T_File *tf = NULL;
    int ch;
    ...
    t_file_set_zero_size(tf);  // Reiniciar el tamaño a cero
    while (EOF != (ch = fgetc(fp))) {
        if (isspace(ch)) {
            continue;
        }

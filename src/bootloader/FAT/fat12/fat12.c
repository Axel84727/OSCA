...
#pragma pack(pop)typedef struct {
    uint16_t entries[FAT12_MAX_CLUSTERS];  // Entradas de la tabla FAT
} FAT12Table;int update_fat(FILE *disk, FAT12Table *fat, BootSector *boot_sector) {
...
FAT12_BootSector boot_sector; // Estructura global del sector de arranque
 ...
 */
int fat12_readBootSector(FAT12_BootSector *boot_sector) {
    ...
    if (fread(boot_sector, sizeof(FAT12_BootSector), 1, disk_file) != 1) {
